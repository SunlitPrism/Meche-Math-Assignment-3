...
function [h_list,g_fel_error_list,g_expmid_error_list, g_bel_error_list, g_analytical_difference, tot_evals_fel,tot_evals_expmid, tot_evals_bel] = global_truncation_error(tspan, hspan, test_function)
    ...
    for i = 1:(length(h_list)) % subtracting by two to make error_list the same size as t_list in asst3. Fix later.
        ...
        %tot_evals_analytical(end+1) = analytical_evals;
    end
