...
function asst3()
    ...
    [global_h_list,global_fel_error_list,global_expmid_error_list, global_analytical_list, tot_evals_fel,tot_evals_expmid] = global_truncation_error(tspan, hspan, @rate_func01);
    ...
    % global_analytical_list(1)
    % global_analytical_list(end)
    ...
    % Plot log scale graph of local truncation error scaling for all four
    % methods
    figure;
    ...
    %loglog(global_h_list,global_bel_error_list,'ro','MarkerFaceColor','r'); hold on;

    loglog(global_h_list,global_impmid_error_list,'go','MarkerFaceColor','g'); hold on;
